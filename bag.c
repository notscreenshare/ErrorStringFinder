...
static gboolean update_loading_animation(gpointer user_data) {
    ...
    GtkTextBuffer *buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(context->result_text_view));
    char loading_text[32];
    context->dots_count = (context->dots_count + 1) % 4;
    
    snprintf(loading_text, sizeof(loading_text), "Loading%.*s", context->dots_count, "...");
    gtk_text_buffer_set_text(buffer, loading_text, -1);

    return G_SOURCE_CONTINUE;
}

gboolean is_valid_memory(const char *buf, size_t len) {
...
void search_memory(SearchContext *context) {
    context->is_searching = TRUE;
    context->dots_count = 0;
    context->timer_id = g_timeout_add(500, update_loading_animation, context);

    char path[256];
    snprintf(path, sizeof(path), "/proc/%d/mem", context->pid);
    
    FILE *maps = fopen(path, "rb");
    if (!maps) {
        context->is_searching = FALSE;
        g_print("Error: Cannot access process memory\n");
        return;
    }

    GString *result = g_string_new("Found:\n");
    ...
    while ((bytes_read = fread(buffer, 1, sizeof(buffer), maps)) > 0) {
        for (guint i = 0; i < context->patterns->len; i++) {
            SearchPattern *pattern = &g_array_index(context->patterns, SearchPattern, i);
            
            for (size_t j = 0; j < bytes_read; j++) {
                if (j + strlen(pattern->search_string) <= bytes_read &&
                    memcmp(buffer + j, pattern->search_string, strlen(pattern->search_string)) == 0 &&
                    is_valid_memory(buffer + j, strlen(pattern->search_string))) {
    ...
    if (!found_any) {
        gtk_label_set_text(GTK_LABEL(context->result_label), "Not Found");
    } else {
        gtk_label_set_text(GTK_LABEL(context->result_label), "Result:");
    }

    GtkTextBuffer *bufferr = gtk_text_view_get_buffer(GTK_TEXT_VIEW(context->result_text_view));
...
static void *search_thread_func(gpointer user_data) {
    SearchContext *context = (SearchContext *)user_data;
...
void on_find_button_clicked(GtkButton *button, gpointer user_data) {
    ...
    GtkWidget *result_text_view = GTK_WIDGET(g_object_get_data(G_OBJECT(button), "result_text_view"));

    gchar *selected_process = gtk_combo_box_text_get_active_text(box_process);
    if (!selected_process) {
        g_print("Invalid data!\n");
        return;
    }

    int pid;
    sscanf(selected_process, "%*[^(](%d", &pid);
    ...
    if (text_in_box && *text_in_box != '\0') {
        GArray *patterns = g_array_new(FALSE, FALSE, sizeof(SearchPattern));
        gchar **lines = g_strsplit(text_in_box, "\n", -1);
        gboolean valid = TRUE;

        for (int i = 0; lines[i] != NULL; i++) {
            gchar **parts = g_strsplit(lines[i], "---", 2);
            if (parts[0] && *g_strstrip(parts[0]) != '\0' && parts[1] && *g_strstrip(parts[1]) != '\0') {
                SearchPattern pattern = {
                    .name = g_strdup(g_strstrip(parts[0])),
                    .search_string = g_strdup(g_strstrip(parts[1]))
                };
        ...
        if (valid) {
            SearchContext *context = g_new(SearchContext, 1);
            context->result_window = result_window;
            context->result_label = result_label;
            context->result_text_view = result_text_view;
            context->pid = pid;
            ...
            GThread *search_thread = g_thread_new("search_thread", search_thread_func, context);
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    gtk_widget_set_size_request(GTK_WIDGET(box_strings), 400, 250);
    ...
    gtk_widget_set_size_request(GTK_WIDGET(find_b), 100, 30);
    gtk_fixed_put(GTK_FIXED(fixed), find_b, 200, 470);
